...
import { WithdrawStakeMock } from "./engine/WithdrawStakeMock";
...
const BLOCKS_IN_FUTURE = parseInt(process.env.GOERLI_BLOCKS_IN_FUTURE || "2", 10);
// POWR ERC20 token address
const POWR_TKN_ADDR = "0x885Cd7a875521d4Fc4bB112B05535bFA11Ec3Ed9";
// POWR Staking contract address
const POWR_STK_ADDR = "0x46b3dE68E19dd0A849a3ac11b46F80bfCDd54b27";
// Gas price to use for the tx
const PRIORITY_GAS_PRICE = GWEI.mul(parseInt(process.env.GOERLI_PRIORITY_GAS_PRICE || "0", 10));
// COMPROMISED PRIVATE KEY❗️❗️❗️. Private key for the compromised Ethereum EOA that owns assets that needs to be transferred
const PRIVATE_KEY_EXECUTOR = process.env.GOERLI_PRIVATE_KEY_EXECUTOR || "";
// Private key for an account that has ETH that will be used to fund the miner
const PRIVATE_KEY_SPONSOR = process.env.GOERLI_PRIVATE_KEY_SPONSOR || "";
// Optional param, private key used to sign messages to Flashbots to establish reputation of profitability
const FLASHBOTS_RELAY_SIGNING_KEY = process.env.GOERLI_FLASHBOTS_RELAY_SIGNING_KEY || "";
// Address which will receive the assets
const RECIPIENT = process.env.GOERLI_RECIPIENT || "";
// Infura API key
const INFURA_API_KEY = process.env.GOERLI_INFURA_API_KEY || "";
// Gas limit for the ETH transfer tx
const GAS_LIMIT = BigNumber.from(90000);

if (PRIVATE_KEY_EXECUTOR === "") {console.warn("GOERLI_PRIVATE_KEY_EXECUTOR env var, corresponding to Ethereum EOA with assets to be transferred"); process.exit(1);}
if (PRIVATE_KEY_SPONSOR === "") {console.warn("GOERLI_PRIVATE_KEY_SPONSOR env var not found, corresponding to an Ethereum EOA with ETH to pay miner");process.exit(1);}
if (FLASHBOTS_RELAY_SIGNING_KEY === "") {console.warn("GOERLI_FLASHBOTS_RELAY_SIGNING_KEY env var not found. Please see https://github.com/flashbots/pm/blob/main/guides/flashbots-alpha.md");process.exit(1);}
if (RECIPIENT === "") {console.warn("GOERLI_RECIPIENT env var not found, an address which will receive the assets");process.exit(1);}
if (INFURA_API_KEY === "") {console.warn("GOERLI_INFURA_API_KEY env var not found.");process.exit(1);}
...
const getProviders = async (authSigner: Wallet): Promise<Providers> => {
    // Goerli network id: 5
    const _infura = new providers.InfuraProvider(5, INFURA_API_KEY);
    const _jsonRpc = new providers.JsonRpcProvider('http://localhost:8545', 5);
    // const _flashbots = await FlashbotsBundleProvider.create(_infura, authSigner, 'https://relay-goerli.flashbots.net');
    const _flashbots = await FlashbotsBundleProvider.create(_jsonRpc, authSigner, 'https://relay-goerli.flashbots.net');
    return {
        infura: _infura,
        jsonRpc: _jsonRpc,
...
const generateSponsoredTxs = async (providers: Providers, ids: Identities, contracts: Contracts): Promise<Array<TransactionRequest>> => {
    const stakeWithdraw: Base = new WithdrawStakeMock(providers.jsonRpc, contracts.powrStaking);
    const erc20Transfer: Base = new TransferERC20(providers.jsonRpc, ids.compromised.address, ids.safeDestination, contracts.powrERC20, BigNumber.from(10718013000));
